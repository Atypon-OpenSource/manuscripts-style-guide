...
export const metadata = (item: BibliographyItem): string => {
  return [authors(item), item['container-title'], issuedYear(item)]
    .filter(Boolean)
    .join(', ')
}

export const issuedYear = (item: Partial<BibliographyItem>): string => {
  if (
    !item.issued ||
...
export const authors = (item: BibliographyItem): string => {
  if (!item.author) {
    return ''
  }
